...
struct ModelConfig {
    ...
    int cpu_moe_layers = 0;             ///< EXPERIMENTAL: layers whose ROUTED
                                        ///< EXPERT tensors go to the host
                                        ///< (0 = off; see decision #75)
...
42. **MTP speculative decode is a target-owned, shared-KV path — separate from the gh#36 separate-draft kernel (#38–41), which stays the disabled/legacy path.** The MTP head (`mtp-gemma-4-E{2,4}B-it.gguf`, arch `gemma4-assistant`, ~57 MB) shares the target trunk's KV via `cparams.ctx_other = ctx_` (`LLAMA_CONTEXT_TYPE_MTP`, `n_rs_seq=0`). Only the holder of `ctx_` can create that context, so the **target backend owns the head lifecycle**: `setup_mtp_draft` builds it lazily against the live `ctx_`; `teardown_mtp_draft` runs before `ctx_` is freed in `do_deactivate`/`do_unload` (the head borrows `ctx_` via `ctx_other`, gh#58). The decode loop mirrors llama.cpp's server MTP consumer: prefill the **target only**, calling `common_speculative_process` on the prompt batch to seed `pending_h`, then per round `draft → llama_decode(ctx_) → common_speculative_process → common_sampler_sample_and_accept_n → common_speculative_accept`. The caller **never** decodes the head context — the impl owns every head decode (shared-KV ⇒ `process()` skips its catch-up decode); no checkpoint dance (shared-KV Gemma 4 is PART-seq_rm). Lossless by construction at `temperature=0` (only the target's own greedy argmax is ever accepted). The orchestrator routes `inference.speculative.mtp` through `try_mtp_route` **before** the gh#36 compat gate (#39/#41) — MTP tolerates the shared-KV Gemma 4 target the gh#36 gate rejects — and `activate_draft` skips the separate-draft secondary backend under MTP. `GenerationResult.n_drafted`/`n_accepted` expose engagement. MTP is the **experimental** speculative path. **MTP's win tracks the RESIDENT FRACTION, not the card — measured at last (v2.13.0, gh#153), replacing the superseded Pascal sentence.** The old claim — "on Pascal, lossless+functional but bandwidth-bound (low accept-rate, no net throughput win)" — is RETIRED, and it was wrong about its own mechanism as well as its number: **the accept rate was healthy in both configurations we measured**, so "low accept-rate on Pascal" describes nothing that happens. Configuration, per #59's first rule: GTX 1080 Ti (Pascal), `n_draft=4`, `flash_attn` on, `q4_0`/`q4_0` KV, `temperature=0`, thinking off, one prompt, four arms (`plain`, an IDENTICAL `control`, `mtp`, `mtp_grammar`) on one model load in a Williams-balanced order, 4 measured trials after a discarded warm-up round, every figure taken from the engine's own generation records. **(a) Fully resident — Gemma 4 E4B QAT + `mtp_e4b`, all 42 layers on the card (3803 MiB VRAM after load), `max_tokens=512`:** plain 53.11, control 53.09, **mtp 75.84 tok/s** (accept 1170/3532 = 0.331), `mtp_grammar` 75.21 (accept 1187/3456 = 0.343, tier grammar `gh153_review` resolved from the records, `source=tier`). Same-config floor **0.045 %**, effect **+42.8 %**. **(b) Partially resident — Gemma 4 26B-A4B QAT + `mtp_a4b` at `gpu_layers: auto` → 18 of 30 layers (13588 MiB model, 10448 MiB free VRAM; trunk split CUDA0 7864.78 MiB / CPU_Mapped 6105.11 MiB), `max_tokens=256`:** plain 18.86, control 18.68, **mtp 18.68 tok/s** (accept 673/1424 = **0.473**), `mtp_grammar` 18.16 (accept 664/1464 = 0.454). Floor **1.00 %**, effect **−0.92 %**. **So MTP pays fully resident and does not pay under partial offload, and the reason is not drafting.** The A4B's accept rate is HIGHER than the E4B's: 2.89 target tokens per forward pass against 2.32, which on drafting quality alone makes it the LARGER of the two expected wins (≈ +47 % scaling the E4B point by the draft headroom, and more on an optimistic reading). It measured ≈ 0 %. **The leading explanation is a HYPOTHESIS, not a measured mechanism, and is recorded as one:** verification cost scales with the drafted batch on CPU-resident layers — a batch of `n_draft + 1` is nearly free on GPU-resident layers, where a decode step is bandwidth-bound on the weights and the extra rows ride along in the same read, but costs roughly the batch size on the 13 layers running on the host, and on a MoE the drafted tokens can route to DIFFERENT experts than the single token would have, multiplying the host-side work again. The one part that IS measured is negative: the MTP head was itself fully GPU-resident on that run (5/5 layers offloaded, 144 MiB CPU_Mapped), so a CPU-resident drafter is ruled out. The `n_draft` × resident-fraction sweep that would confirm or kill the rest is **gh#167**; until it runs, the claim here is the two numbers and the residency they were taken at, never the reason. **What these numbers do NOT cover:** (i) `throughput_tok_s` stamps its clock BEFORE tokenize and prefill on the plain and the MTP path alike, so every figure above is **tok/s over the generate call**, not a decode rate — the arms are comparable because prefill is identical between them, but the absolute number is not decode-only; (ii) **CUDA graphs are forced OFF in every entropic build** (`ENTROPIC_CUDA_GRAPHS` defaults OFF and forces `GGML_CUDA_GRAPHS` off in `extern/CMakeLists.txt`), so the MTP draft-path speedup upstream #28549 added — the one #60 bumped the pin partly to reach — is NOT in these numbers, and a graphs-enabled build would have to re-measure; (iii) expert offload (`-ncmoe` / `--n-cpu-moe`) exists upstream at this pin and entropic did not expose it when these numbers were taken, so the A4B split moves WHOLE layers, attention included, and says nothing about an experts-only split — the `cpu_moe_layers` prototype of #75 landed in the same version, default off, and is NOT in any figure above; (iv) 4 trials, one prompt, one card, one `n_draft`, `temperature=0` — nothing is claimed about other `n_draft` values, other resident fractions, or other architectures. The suite asserts this under the resident-fraction rule of **#74**, which is why neither figure can quietly become a hardware property again. Exercised greedy on Q8 E2B and on mobile-QAT TQ2_0 (E2B + E4B). **Lossless holds only at `temperature=0`** (greedy argmax accept); see #43 for the v2.9.1 envelope guards that enforce this. (v2.9.0, gh#106; performance record measured and rewritten v2.13.0, gh#153/gh#167)
...
66. **Serialization was an accident of the run guard, so removing it had to make the serialization deliberate — and the audit is the feature, not the lock (v2.13.0, gh#158).** `ENTROPIC_ERROR_ALREADY_RUNNING` was a per-HANDLE compare-exchange (#58/gh#144), so a host serving several conversations from one resident model serialized every turn — and the consumer's measurement is the argument: 123 tok/s at one sequence against 353 at eight on the same weights via llama-server, because decode is bandwidth-bound and one weight read serves a whole batch. **But nothing else serialized `llama_decode`.** One `llama_context` is shared by every session and tier on a GGUF; `LlamaCppBackend::active_slot_`, `residency_`, the staged tool arena and the sampler chain are all ONE field per backend; `entropic_run_batch` opened by clearing the WHOLE KV cache and held a local cancel flag nothing in the tree could set. So the per-handle guard was not merely a policy about turns — it was the only thing making every one of those fields safe, and keying it per session removes that protection from all of them at once. **Three consequences, recorded in the order they matter.** **(a) The lock.** `ModelOrchestrator::generation_mutex_` is the ONE lock on the generation path, held across a whole `generate*` call rather than around `llama_decode` — which is what makes concurrent runs interleave at GENERATION boundaries (a side request waits for the current generation, not the whole multi-iteration turn) and what covers `active_slot_` and the arena, which a decode-only lock would leave racing. Order: `generation_mutex_` → `swap_mutex_` / `records_mutex_` → the backend's `transition_mutex_` / `mtp_mutex_`. It is **impossible to invert by construction rather than by discipline**: the lock is taken at exactly five sites, all entry points of one class, and no holder of an inner lock calls any of them — `get_model` takes `swap_mutex_` and RELEASES it before returning, so nobody holds it across a generation. It is RECURSIVE because `generate()` routes and `route()` → `classify_task()` decodes on the router, which is itself a generation entry point; a plain mutex would self-deadlock the first time a run routed. **(b) The default is off.** `concurrent_sessions` ships `false`. Per-key runs are safe only once every piece of per-handle mutable state a turn touches is per-run or locked, and shipping the concurrency on would make every existing consumer concurrent without their asking — the mistake #63/gh#157 explicitly refused to repeat with `keep_warm`. A racy default is strictly worse than today's honest serialization, which is also why the flag is the ONLY thing that changes behaviour: with it off the guard is handle-exclusive and v2.12.0 semantics hold exactly. **(c) The bug the audit found was the binding, not the lock.** The session key was a per-handle field read at `run()`; making it thread-local so two runs can each answer "which session am I" produced a subtler defect — a binding that outlived its run stayed on that thread and silently captured the NEXT unkeyed run, on any handle. The existing gh#144 tier-switch test caught it, because Catch2 re-runs a scenario per leaf section and the second run inherited the first's binding. The fix is that **the claim IS the binding**: `try_begin_turn` installs it and `end_turn` removes it, so there is no state whose lifetime is not a run's. **External-tool cancellation becomes per REQUEST for the same reason.** gh#150's `interrupt_external_tools()` trips every transport at once, which is right for "stop everything" and wrong for "stop session A" — it would hand session B an empty tool result indistinguishable from a real one, which is the defect gh#150 was filed for, arriving through a different door. Threading a token through would change `ToolExecutionInterface`, `ServerManager`, `ExternalClient` and `Transport` — four signatures across three `.so` boundaries — for a value already unambiguous from context: a run is a blocking call on the caller's thread and every tool call it makes happens on that thread. So the token is published per THREAD (`RunCancelScope`, modelled on `HandleLogScope`) and the transport polls it beside the latch. `entropic_interrupt` keeps the latch; `entropic_interrupt_session` never touches it. **And the batch.** `run_batched_decode`'s whole-cache clear and its hard-coded sequence 0 for arm 0 are both invisible to a correctness test — a session whose KV was destroyed still answers correctly, having silently paid a cold prefill — so the fix ships with a counter (`kv_full_clear_count()`, the instrumentation shape of #62/gh#161 and #65/gh#148) and a pure planner the production path consults, because a helper the code does not call proves nothing. **COMPLETING THE AUDIT, AND WHY THE DEFAULT IS NOW ON (v2.13.0, second pass).** (b) above said the flag ships off *until every piece of per-handle mutable state a turn touches is per-run or locked*. That is a finite list, so it was finished rather than deferred, and the default moved with it — `concurrent_sessions` now defaults `true` and remains as a KILL SWITCH, the one-line escape hatch for a consumer who hits a concurrency defect in the field. Resolved in this pass: **`token_counter_`** — its memo was an `unordered_map` keyed by the ADDRESS of a `Message`, written from a `const` method with no lock. Two defects in one field: an address is not an identity (the message vector reallocates, the allocator reuses slots, so a hit could answer for a message that no longer exists) and the unsynchronized insert is heap corruption rather than a wrong number. *Deleted*, not locked — `count_text` is one `size()` and one division, strictly cheaper than hashing a pointer, so the memo was never worth defending. **`per_tier_metrics_` / `last_metrics_`** — `metrics_mutex_`, and the fold was copy-pasted at two sites (`run()` and `run_loop()`'s tail) so it was extracted first; locking one would have left the other racing. `entropic_metrics_json` copied the map while a finishing run inserted into it, and an insert that rehashes relocates every bucket under the copy's iterators. Its `generations[]` ring (gh#154) was ALREADY correct — `records_mutex_`, returning a copy — which is the useful contrast: the field added THIS release was safe and the accumulator that has been there since v2.0.6 was not. **`context_anchors_`** — `anchors_mutex_`; handle-wide, but written from INSIDE the loop by the `context_anchor` directive. **`system_prompt_`** — `prompt_mutex_`; written on the API thread under `api_mutex` and read on run threads, which have held nothing since gh#109 (v2.9.5) took `api_mutex` off every run entry point, so `api_mutex` never covered that pair. **`sandbox_mgr_` + the repo-dir cache** — `sandbox_mutex_`, as ONE unit because the manager is constructed from the cached path. The check-then-emplace was a use-after-free, not a wasted snapshot: two delegating runs both see an empty `optional`, both `emplace`, and the second destroys the manager the first already returned. `SandboxManager::ensure_base_snapshot` has the same shape one level down and gets its own `base_mutex_` — two runs both copying the project tree into one `base_dir_` is a file-level race, not duplicated effort, and the lock is held ACROSS the copy because the second caller wants the finished snapshot rather than its own. **`ConstitutionalValidator`'s per-turn fields** (`current_tier_`, `current_tool_context_`, `current_tool_evidence_`, `current_system_prompt_`) — the sharpest find, and one the brief did not name: per-HANDLE members describing a per-TURN thing, set at `handle_hook()` entry. A mutex fixes the `std::string` race and leaves the real defect, which is session A's critique prompt built from session B's tool evidence and identity prompt. Made thread-local; the one path that legitimately crosses threads, `resume_retry()` from the consumer's thread after `paused_pending_consumer`, carries the system prompt forward in `PendingValidationState`. **The decode itself** — and this is the gap the first pass left in its own work. ba48877 gave each run a cancel token and taught the agent loop and the MCP transports to poll it, but `ResponseGenerator` holds `GenerationEvents{&interrupt_flag_, &pause_flag_}`, raw pointers to the HANDLE-wide flags fixed at construction, so the per-token poll and the batch cancel poller answered only to `entropic_interrupt()`. `entropic_interrupt_session(A)` therefore could not stop the generation it named — the longest-running thing a session does. Both sites now OR in this run's token (`generation_cancelled`), and the batch poller, which runs on its OWN thread where the thread-local is not installed, captures the token pointer on the run thread before spawning. The pause prompt's "no injection" path likewise raises THIS run's token (`cancel_current_run()`) instead of the handle-wide flag, which used to abort every concurrent session on one session's declined prompt. **`pause_flag_` stays handle-wide, deliberately.** The asymmetry with the interrupt is the argument: `interrupt_session(key)` can route because the engine owns the map from key to run, whereas `pause()` is raised from a thread that owns no run and names no key — routing it would need an `entropic_pause_session` that no consumer has asked for, and every consumer of pause today is a single-session TUI holding one conversation. What WAS wrong is that a fresh turn cleared it unconditionally, so starting run B silently un-paused run A: the same shape as the interrupt defect this issue was filed for. The clear is now conditional on being the only run in flight, which is byte-identical behaviour under `concurrent_sessions: false`. **TOOL EXECUTION, which was not in the brief and is the finding worth reading twice.** `StdioTransport::send_request` has held `io_mutex_` across write-then-read since v1.8.7, so two concurrent runs never interleave BYTES on the pipe — the obvious hazard was already handled. The real one is that **nothing ever checked that the response line answered the request line.** A request can be ABANDONED — a timeout, or `request_cancelled()` tripping mid-read, which gh#158 turns from an exceptional event into a routine one — and the server still answers; that answer sits in the pipe and is read as the result of the NEXT request. Serialized, that was a stale answer to yourself. Keyed, it is a cross-session leak of exactly the gh#150 shape: session A's interrupt hands session B the content of A's tool call, and B cannot tell. Two fixes, because one of them is recovery and the other is detection: the transport marks the pipe desynced on an abandoned read and drains orphaned lines before the next write, and `ExternalMCPClient` now stamps and VERIFIES the JSON-RPC id, returning a typed error rather than another request's payload. `next_id_` became atomic in the same pass — a plain `int++` from two run threads (and from the HealthMonitor's reconnect thread, which has issued `initialize`/`tools/list` through it since v1.8.7) is a data race, and two requests sharing an id is precisely what defeats the pairing check. **What was checked and deliberately NOT changed:** `ServerManager`'s four registry maps have no lock, but two concurrent runs only READ them, and concurrent reads of a `std::map` are safe. The writer is a runtime `entropic_connect_mcp_server` / `disconnect` landing while a turn is in flight — an API-versus-run race that has existed since gh#109 (v2.9.5) removed `api_mutex` from the run path, is not made worse by keying the guard, and whose honest fix is `shared_ptr` ownership so `execute()` can hold a reference across a call it must not hold a lock across. Recorded here rather than half-fixed. `ToolExecutor` needed nothing: its only shared mutable state is `history_` (its own `shared_mutex`) and `history_seq_` (atomic); everything else is `LoopContext`, which is per-run already. **ThreadSanitizer is the proof, and getting it to run was itself a finding.** The `tsan` preset existed and could not build: on Linux >= 6.5 the kernel hands out 32 bits of mmap entropy and GCC's TSan aborts with `unexpected memory mapping` before `main()`, so `catch_discover_tests` — which RUNS each test binary at build time — failed the build outright. `sysctl -w vm.mmap_rnd_bits=28` fixes it and needs root; `setarch -R` disables ASLR for the process and needs nothing, so the preset now sets `CMAKE_CROSSCOMPILING_EMULATOR=/usr/bin/setarch;-R`, which both Catch2 and ctest honour, scoped to that preset alone. The run found two races the CPU suite cannot see, and neither was in the state this audit set out to lock. `StdioTransport::close()` closed the three fds and THEN joined the stderr pump, so `close_fd` wrote `stderr_fd_ = -1` while the pump was still reading it — a data race on an int and a use-after-close waiting to happen, since a freed fd number is immediately reusable by another thread's `open()`; the join now precedes the closes. And `MockInference`, the test HARNESS, raced on `generate_call_count++` and `response_queue.erase()` once one engine served two runs — worth recording because, left alone, a harness race reads as a finding about the engine. After both, 15/15 gh#158 tests pass with zero warnings, and the newly locked engine state is clean. **THE BUILT-IN SERVERS' OWN STATE (v2.13.0, third pass).** The first two passes locked the engine and the external transports and never opened the in-process servers — and `concurrent_sessions: true` means every UNBOUND session on a handle shares its ONE default server set, so two sessions can be inside the same `FilesystemServer` at the same moment. Three findings, and the one worth reading twice is a wrong answer, not a crash. **(1) `FileAccessTracker` had no lock.** `read_file` inserts into an `unordered_map` that `write_file` / `edit_file` look up; TSan: 7 races in `record_read`. A leaf mutex inside the tracker. **(2) The sandbox swap was exclusive against every other swap and against nobody's tool calls.** gh#160's facade `recursive_mutex` was taken by `ScopedSandbox` entry and restore only, so while session A's isolated delegation had the default set re-rooted, session B's `read_file("notes.md")` and `bash cat notes.md` resolved INSIDE A's sandbox — deterministically, the test reads `MARKER-sandbox` back — and B's writes would have landed there and shipped in A's patch. The swap itself was also an unsynchronized `std::filesystem::path` write plus an `IgnoreMatcher::load` racing B's `resolve_path` / `is_ignored` / bash's default cwd (4 TSan races in the deterministic scenario, 13 under swap churn, where all 30 of B's read batches saw the sandbox in at least one of their two calls). So the answer to "does B wait, read A's sandbox, or race?" was *both of the last two*. **The fix is `ToolRootLock` on the `ServerManager`**: SHARED around every in-process and plugin dispatch, EXCLUSIVE from sandbox entry to restore (`enter_working_dir` / `leave_working_dir`, and `set_working_dir_all` for its own duration), re-entrant for its owner, and the owner passes through its own shared side — the sandboxed child's calls are the ones that SHOULD see the sandbox. B now WAITS, and then resolves against its own root; the wait is logged by tool name before and after, so a stall behind another session's delegation is not mistaken for a slow tool. External servers take nothing — the swap never moves them. The facade's `sandbox_swap_mutex` and `EntropicWorkspace::swap_mutex` are deleted: one lock, living with the state it guards, and one per `ServerManager` is still one per workspace. **The honest cost**: under `delegation.isolation: sandbox` (default `none`) other sessions' in-process tool calls on the SAME server set stall for the length of a sandboxed delegation. The parallel path is a workspace per session (gh#166). A per-call thread-local root would give the built-ins back their parallelism but not the dlopen plugins, which hold their own cwd behind a C ABI that cannot see a thread-local — two mechanisms for one invariant, so not done. **Lock order**: `ToolRootLock` is OUTERMOST — exclusive, it is held across a child's whole run, so `ToolRootLock → generation_mutex_ → swap_mutex_ / records_mutex_ → transition_mutex_ / mtp_mutex_`; shared, it is taken only at tool dispatch on a run thread that holds none of those nor `api_mutex`, and nothing inside a dispatch reaches a generate entry point, so no inner-lock holder ever waits on it. No shared→exclusive upgrade can happen on one thread: a delegation starts after its tool batch returns (`dispatch_pending_or_halt`), never inside `ServerManager::execute`. **(3) `TodoTool::items_` — outside the three servers the brief named, found by the same sweep.** The entropic server keeps its todo list on the tool object, one per server set; two sessions appending raced `std::vector::push_back` and the TSan run ABORTED with an impossible allocation size inside it — heap corruption. The action and the render now run under one hold of a leaf mutex. **Two semantic questions are left open, deliberately, because they are design and not locking:** the read-before-write tracker and the todo list are per SERVER, so session B may overwrite a file only session A read, and B's todo result lists A's items — the per-handle-describing-a-per-session-thing shape the validator fields had, now locked but not re-scoped. **RE-SCOPED PER SESSION (v2.13.0, fourth pass — the user's decision on those two questions).** Both are now keyed by the running session. **The key is the one the call ROUTED on**: `ToolExecutor::execute_tool` publishes `LoopContext::session_key` to its own thread (`RunSessionScope`) for the duration of the dispatch, and the tracker and the todo tool read `current_run_session()` — so the state is filed under exactly the value that chose the server set, rather than a second notion of "which session" that could drift from it. `HandleTurnGuard` already publishes the same key for the whole run; republishing it at dispatch is what makes a direct `ToolExecutor` driver (the white-box tests) and a delegated child agree with production by construction. **A delegated child shares its parent's key** (`build_child_context` / `build_resumed_child_context` copy it), so a child's read legitimately unlocks the parent's write — kept deliberately, and pinned by a test that drives a real `DelegationManager`. **Bounded growth**: a session's entries are released when its conversation ends — `entropic_session_drop`, `entropic_session_context_clear`, `entropic_session_context_set` (a restore replaces the history the reads described) and `entropic_context_clear` (the active session) — through `release_session_tool_state`, which visits the default set AND every workspace's, because which set a session touched depends on when it was bound and releasing an absent key is free. The model-facing argument for releasing on clear is the same as the correctness one: read-before-write means *this conversation has seen the file*, and a cleared conversation has not. **Mechanism**: a `SessionScoped<T>` map (one leaf mutex, a `peek` that never allocates — asking "did this session read X?" must not grow the map for sessions that never read) and a `SessionStateOwner` mixin that `ServerManager::release_session` cross-casts to. **Not a virtual on `MCPServerBase`**: its vtable is plugin ABI (`i_mcp_server.h`), so a server opts in beside the base instead. `release_session` takes no `ToolRootLock` — the state is behind its own leaf lock, and waiting on the root lock would stall an API call for a whole sandboxed delegation. **Checked while here, not changed:** `AgentEngine::context_anchors_` is handle-wide and re-injected at every run start, which would leak one session's `file:` / `todo_state` anchors into another's turn — but tool-emitted anchors never reach it (`build_directive` returns nullptr for `context_anchor`, so the directive is dropped), so there is nothing to leak today. If that path is ever wired, the map must become per session first. `DelegationManager::set_todo_callbacks` (save/fresh/restore around a child) has no caller in `src/`, so a child's todos land on the parent session's list — the pre-existing behaviour, now per session.
    ...
    | bash | `timeout_`, the tool | ctor (`mcp.bash.timeout_seconds`, #73) | immutable; ENFORCED since #73 (was never read) |
...
73. **A tool that runs arbitrary shell commands had a timeout on paper and a safety claim in a comment, and neither was in force (v2.13.0).** `BashServer::timeout_` was stored, logged and never read — `popen()` offers no handle to stop a command — so `sleep 30` held the tool call, the run thread and every lock above it for 30 s, and nothing could be configured because `init_builtins` never passed a value either. `bash.h` meanwhile said the server "blocks dangerous commands" and "enforces timeout". **Both claims are now honest.** The timeout is ENFORCED: each command is `/bin/sh -c` spawned (`posix_spawn`, `POSIX_SPAWN_SETPGROUP`) as the leader of its OWN process group; on the deadline the whole group is `SIGKILL`ed and the shell reaped, and the model gets a typed `{"error":"timeout", message, timeout_seconds, elapsed_ms, output}` naming the limit and the elapsed time and keeping the partial output — a bare "timed out" invites a retry of the same command. The exit is detected with `waitid(WNOWAIT)` so the shell stays a zombie, and therefore keeps its pid — which IS the group id — until after the group kill: a reaped leader's pid could be reused, and the kill would land on an unrelated group. **The same kill runs on a NORMAL exit**, because the requirement is that a tool call leaves no processes behind: `cmd &` used to outlive the call, and a background child that inherited stdout held the read loop open for its whole life (measured: `sleep 25 & echo` took 25 s to return; now 0.01 s, child dead). A process that deliberately leaves the group (`setsid`) escapes by design, and its output is abandoned after a 2 s drain grace with a warning. **The denylist claim is deleted, not implemented.** Nothing ever filtered commands, and a pattern denylist on free-form shell is trivially bypassed; command gating is the permission layer (`permissions.allow`/`deny`, keyed by `get_permission_pattern`) and tool approval, and the header now says so. **Two side effects, both deliberate:** stderr is captured for EVERY command, not only where the command's own trailing `2>&1` reached (a non-final command's stderr used to go to the host's terminal), and stdin is `/dev/null` (a tool call is not interactive and must not read the host's terminal). **The implementing session's call, recorded so it can be overruled:** `mcp.bash.timeout_seconds` (default 30, the value the constructor always had; `>= 1` or the config load fails — no silent clamp, and 0 does not mean "unbounded") is NEW. The brief assumed a configured timeout existed; none did, and enforcing a hard-coded 30 s with no knob would turn every build or test suite run through bash into a kill. `mcp.server_timeout_seconds` was NOT reused: it has been parsed since v1.8.1 and read by nothing, its name does not say which server, and giving a vague key a specific meaning would be a second undocumented behaviour — it is left inert and labelled so in `config.h`. **Not done:** the bash call does not yet poll the per-run cancel token (`current_run_cancelled()`, #66), so `entropic_interrupt_session` still waits for a running command up to its timeout — the timeout is now the bound that interrupt previously did not have.

74. **A performance assertion is gated on the condition the performance depends on, not on which test is running (v2.13.0, gh#153).** The gh#153 matrix measured MTP at **+42.8 %** on a fully GPU-resident E4B and **−0.92 %** on an A4B at 18 of 30 layers, on the same card, the same engine and the same `n_draft` (#42). One pinned threshold would have to be wrong for one of them, and the tempting fixes are both bad: assert the win only in the case that shows it (the rule then encodes which model is under test, not what MTP does), or lower the bar until both pass (the E4B stops being a regression test). The rule is instead SELECTED, per run, from what the engine RESOLVED: **fully resident → MTP must CLEAR the same-config floor; partially resident → MTP must only not be materially WORSE than plain, and no win is claimed.** Residency is read from `gpu_layers` AFTER admission — where `auto` has already become a number (#65) and `-1` still carries llama.cpp's "every layer" — against `llama_model_n_layer`; the YAML string is never consulted, because `-1` and `auto` state an intent and only one of them survives contact with the card. A layer count that cannot be read fails the run rather than selecting the weaker rule (#43's fail-loud principle: guessing the gate is how a gate stops being one). **Amended (v2.13.0, gh#153 #42(iii)): residency is about WEIGHTS, and a layer count alone cannot report it.** `cpu_moe_layers` (#75) places a layer's routed-expert FFN tensors on the host while that layer's attention, KV, router, shared/dense FFN and norms stay on the card, so a tier at `gpu_layers: 30, cpu_moe_layers: 30` reports **30 of 30 layers resident while the large majority of a 26B-A4B's bytes are in system RAM** — and the rule as first written would have asserted the STRICT, fully-resident bar on it, which is precisely the misreading this decision exists to prevent. `fully_resident` therefore requires `cpu_moe_layers == 0` as well, and the rule is deliberately coarse: ANY expert offload disqualifies, because "the first N layers" is not "N layers' worth of experts" and a resident FRACTION that counted bytes would need GGUF tensor metadata the engine reads nowhere — a coarse "not fully resident" is honest, an invented fraction would not be. `cpu_moe_layers` is recorded beside every `resident_fraction` (human table, config block, every arm row, `effect`), `offload` carries a `+experts_host` suffix so `full+experts_host` cannot be skimmed as `full`, the printed RESIDENCY block states in words that the experts are host-side, and the summary schema is `entropic.gh153.mtp-bench/3`. BOTH rules' outcomes are computed and printed on every run with the asserted one marked, and the resident fraction is recorded beside every figure in the human table AND in `build/test-reports/bench/gh153_mtp_{e4b,a4b}.json`, because the number means nothing without it — the same A4B on a 12 GB card is still partial, at a better fraction, and must report a BETTER number under the SAME rule rather than a different rule. The tolerance in the partial direction is `max(floor, 5 pp)`, the same quantity as the clearing rule's margin read backwards: on the run that fixed #42 the floor was 1.00 % and the effect −0.92 %, which a bare-floor bound would have passed by 0.08 pp — not a margin, a coincidence. **The general rule behind it, extending #59:** a performance claim must record the configuration it was measured in (#59), and a performance ASSERTION must additionally be conditioned on it — otherwise the gate silently becomes a claim that the condition always holds, which is exactly how #42's Pascal sentence outlived the default it was taken under and read as a property of the hardware for four minor versions. (v2.13.0, gh#153, gh#167)

75. **Whole-layer offload is role-blind, so the prototype that isn't is a config key and a default of OFF (v2.13.0, gh#153 #42(iii)).** `gpu_layers` places a layer wholly on the device or wholly on the host. On a model that does not fit, that spends VRAM on rarely-touched routed-expert weights while exiling attention — **and its KV** — to system RAM. The measurement is #42(b): Gemma 4 26B-A4B QAT (14.25 GB) on an 11 GiB GTX 1080 Ti resolves `gpu_layers: auto` to **18 of 30 layers**, runs at 18.7 tok/s, and MTP — worth **+42.8 %** on the fully resident E4B — is worth **nothing** there (−0.92 % and +0.82 % across two runs against a same-config floor of 0.5–1.0 %). Published practice is unanimous about the direction (KTransformers, Fiddler ICLR'25, mixtral-offloading): keep attention, KV and the shared/dense path resident and push only routed-expert FFN weights to the host. **`models.<tier>.cpu_moe_layers: N` exposes exactly llama.cpp's `-ncmoe` / `--n-cpu-moe`, and is named for it on purpose** — the mechanism (`llama_model_params::tensor_buft_overrides`), the semantic ("the FIRST N layers") and the tensor-name patterns are upstream's, so an operator who has read llama.cpp's documentation transfers it without translation and a future `-cmoe`/`-ncffn` sibling has an obvious name. The pattern tail is **copied verbatim** from the vendored `LLM_FFN_EXPS_REGEX` rather than inferred, and a CPU unit test includes `<common.h>` and asserts the two are byte-identical, because a stale pattern matches nothing and offloads nothing while every other gate stays green. It was checked against the **gemma4** MoE arch specifically, not assumed from DeepSeek naming: at this pin `src/models/gemma4.cpp` creates `ffn_gate_exps` / `ffn_up_exps` (or the fused `ffn_gate_up_exps`) and `ffn_down_exps`, `llama-model.cpp` attaches a per-expert `.scale` to each, and the test runs the assembled patterns through the same `std::regex_search` `llama-model-loader.cpp` uses against those literal names — and against attention, `attn_*_norm`, the router `ffn_gate_inp`, the dense/shared FFN, every `*_shexp`, the norms and the embeddings, none of which may match.

**The lifetime is the trap, and upstream's answer is not available to a library.** `llama_model_tensor_buft_override` holds a `const char*`; llama.cpp copies neither the array nor the strings, storing the pointer and dereferencing it per tensor per pattern throughout placement. `llm_add_n_cpu_ffn_overrides` keeps its strings in a function-local `static std::list<std::string>` that is never freed — fine for a CLI that loads one model and exits, wrong for an engine that loads, unloads and reloads across many handles. `ExpertOffloadOverrides` owns its strings instead, fills them ONCE so no reallocation can invalidate a `c_str()`, appends the `{nullptr, nullptr}` terminator llama.cpp's walk requires, and deletes copy and move so "this object contains pointers into itself" is enforced rather than reasoned about. It is constructed in the scope that performs the load, at both call sites.

**Refusals are split by what can be known when.** Configure time, purely, with the tier named: a negative count (there is deliberately no "all layers" sentinel — `-1` means "every layer" for `gpu_layers` only because llama.cpp defines it so, and nothing defines it here); `gpu_layers: 0` (the whole model is already host-side, so the knob would be a no-op an operator explicitly asked for); and **`gpu_layers: auto`**, because the gh#148 admission math prices a layer as a WHOLE layer and has no model of expert placement, so the split it derived would not describe the load being performed. Load time, because they need GGUF metadata entropic reads nowhere else: a model that declares no experts (`<arch>.expert_count` absent or 0 — otherwise the operator measures the technique instead of their mistake), and a count beyond the block count (not clamped; a count that cannot be honoured is an error, not a preference). The load-time pair costs one wasted read to refuse, which is the right trade against a configuration that quietly placed nothing.

**Deliberately NOT done, and this is the boundary of the prototype.** The residency/admission footprint math is untouched: `estimate_footprint_bytes` and `gpu_layers: auto` still price whole layers, so a prototype user states `gpu_layers` explicitly and that combination is refused rather than approximated. Nothing here is measured — the knob exists so the experts-only split CAN be measured against #42(b)'s 18-of-31 baseline, and whether it ships as a supported feature in v2.13.0 or v2.14.0 is that measurement's to decide. The engagement signal is a load-time INFO line plus llama.cpp's own `load_tensors:` buffer sizes shifting from the device buffer to `CPU_Mapped` at an unchanged `gpu_layers`; at DEBUG llama.cpp names every tensor it overrode. **Absent the key, no override array is built at all and `tensor_buft_overrides` stays the nullptr `llama_model_default_params()` already carries — byte-identical to v2.12.2, which is why no green gate from this release had to be re-run.** (v2.13.0, gh#153)

**The measurement case, and how a prototype knob is held to having done something.** `[mtp-a4b-experts]` runs the same four arms, the same floor and the same rules as `[mtp-a4b]` on the same A4B and the same card, changing one thing: `gpu_layers: 30` with `cpu_moe_layers: 30` (the model has THIRTY layers; the first attempt asked for 31 and the engine refused it rather than clamping, which is the refusal working). The count is explicit for a reason worth writing down — `-1` and `99` classify as `Offload::full` in `vram_footprint.h`, so the gh#148 admission gate prices the whole 14.25 GB file against an 11 GiB card and refuses the tier before the knob can help, and `auto` is refused by design; `30` classifies as `partial_unknown`, which leaves the gate open because the engine declines to price what it cannot price. **A knob that silently did nothing must FAIL, never report a number**, and llama.cpp exposes no post-load per-tensor buffer query, so the case takes the strongest in-process signal and states what makes it sufficient. MECHANISM: `gpu_layers` is asserted to have resolved to exactly `llama_model_n_layer`, so every repeating layer was REQUESTED resident and nothing left in the engine can put trunk weights on the host except the expert override — which rules out both ways the arm could quietly become an ordinary partial-offload run. EFFECT: the VRAM the load actually took (device-wide, after minus before) must clear a bound derived from the trunk's size on disk and this card's free VRAM alone, `bound = free − (weights − free)` — the ceiling, cleared by the overflow again, the same margin idiom as the floor rule's `required = floor + max(floor, 5 pp)`. It is more than an idiom here: `cpu_moe_layers` is not a fitting algorithm, it has no idea how big the card is and moves EVERY routed-expert tensor of the first N layers rather than the least that would fit, so an engaged override lands far below the ceiling and a load that merely squeaked under it did not do what the knob does. Both halves rest on the card being unable to hold the trunk; on a card that CAN, the case SKIPs with that reason before the load rather than passing vacuously. What it does NOT establish is HOW MUCH moved — nothing here prices the split in bytes, and the admission math still models no expert placement. (v2.13.0, gh#153 #42(iii)) **MEASURED 2026-09-22 (1080 Ti, gpu_layers=30, ctx 8192, max_tokens 256, 4 trials), sweeping `cpu_moe_layers` by hand:** N=30 — 2767 MiB VRAM, plain 17.68, mtp 18.18 tok/s, effect −0.29 % (floor 3.11 %); N=20 — 6775 MiB VRAM, plain 22.62, mtp 23.76 tok/s, effect **+4.16 %** (floor 0.83 %); N=12 — 9446 MiB, refused by the engagement bound (see below). **Experts-on-host by itself buys NOTHING on this card**: N=30 lands at 18.2 tok/s against whole-layer offload's 18.7 (#42(b)), because the per-token expert bytes still come from host RAM either way. What it buys is **VRAM** — 2767 MiB against 7865 MiB — and that only pays once the freed memory holds experts again: at N=20 throughput rises to 22.6 and **MTP's effect turns positive (+4.16 %)**, which is #74's resident-fraction rule appearing INSIDE one model rather than between two. **The engagement bound cannot judge a mid-sweep point**: it assumes most weights are host-side, so N=20 clears it by 300 MiB and N=12 exceeds it — not because the knob failed but because 18 layers of experts ARE resident. The committed case therefore stays at N=30, the robust comparison against #42(b); sweeping N needs a residency-aware bound and is gh#167/gh#180's job.

76. **An ABI that advertises a modification channel must apply it, and the guard on it is identity, not content (v2.13.0, gh#168).** `entropic.h`'s memory-ownership paragraph names `hook modified_json` as a cross-boundary allocation the engine consumes, and `types/hooks.h` states the pre-hook contract as "return 0 + `*modified_json != NULL`: proceed with modified context". `ToolExecutor::fire_pre_tool_hook` did `free(mod); return rc != 0;` — so for every tool, on every call, since the hook point existed, the advertised channel was a documented no-op. This is the SAME defect as issue #2 on `POST_TOOL_CALL` (v2.1.1) and, before it, gh#3: the engine freeing a hook's output without applying it. POST was fixed twice and PRE was never looked at, which is why the class is written down here rather than in a third commit message. The consumer symptom was silent by construction — a host appending a parent's verbatim observation to a delegated task saw the specialist work from the lead's paraphrase, with no error anywhere; their feature reached **0 of 15** lifetime and two downstream workarounds measured **0 of 3** each, because neither could reach a channel that was never wired. **Only the ARGUMENTS may change.** The payload is the same object shape the hook received and only its `args` object is read; `tool_name`, if restated, must equal the call's own. A hook that could rename the tool would route a permitted call to a tool the tier's `allowed_tools` never granted — the per-tier dispatch gate (gh#83) reads the name BEFORE the hook, so an identity rewrite lands after the only check that would have caught it. Arguments do not have that property: they are re-validated against the tool's schema, re-keyed for duplicate detection and re-matched against permission patterns after the hook, which is why they are the safe half. **Refusal is loud, total and NOT a cancellation.** A payload that is not valid JSON, not an object, carries no `args` object or names another tool is refused whole — logged at ERROR with the reason and the verbatim payload, with the model's original arguments dispatched. The tempting alternative, treating a bad payload as a hook error and cancelling the call, was rejected: cancellation has exactly one channel (a non-zero return), and overloading it would make a buggy host hook indistinguishable from a deliberate policy denial in both the model's context and the `rejected_precondition` result kind — the host's bug would be diagnosed as the agent's behaviour. A silent fallback was equally rejected (#43): the ERROR line IS the surfacing, and it names the knob the operator has to fix. **Both argument representations are rewritten,** `arguments_json` (what `serialize_args` hands the server, the schema validator and the POST context) and the `arguments` map (what `tool_call_key` hashes), because writing only one would let a rewritten call collide with the original in the duplicate cache and be rejected as a repeat of a call that never ran. `process_single_call` therefore takes the model's `ToolCall` by const reference and works on its own copy, so preconditions, dispatch, the dup cache and the POST hook all observe one consistent call. **The payload is sanitized BEFORE it is parsed,** not after: a hook is a plugin `.so`, an external boundary in both directions (gh#3, gh#111, gh#132), and nlohmann rejects an ill-formed UTF-8 byte inside a string — sanitizing after the parse would refuse a payload that is merely dirty and turn the gh#113/#114/#132 trap into a second silent no-op.

77. **The iteration cap is an ANNOTATION on the run's last real output, never a replacement for it (v2.13.0, gh#169).** `AgentEngine::loop` pushed a synthetic assistant message whose CONTENT was `[iteration cap reached after N iterations — returning current state]`. `DelegationManager::extract_summary` takes the last assistant message as `DelegationResult::summary`, so on every capped delegation that placeholder became the child's result — in the parent-tier relay, in the ON_DELEGATE_COMPLETE payload, and in the storage record. The child's real summary EXISTS at cap time: the consumer's curriculum specialist completed its work at iteration 10 of 10 with real text and the parent was handed the placeholder instead. **The measured symptom is a lead confidently reporting the opposite of what happened:** in 3 of 5 turns the lead told a parent a record lookup had FAILED while `get_student_record` had returned `status=ok` with a full 501-character record and `set_interest` had written the row; separately a lead apologised that a lesson plan "didn't come through" with the finished lesson sitting in the store. The engine was not losing the work — it was overwriting the one field the parent reads. **The cap now carries the last substantive assistant content and appends the note**, so the parent gets the answer AND the fact that it is partial. "Substantive" is byte-level (any content that is not entirely whitespace), measured before the cap message is pushed, so there is no engine-authored marker to filter out and no classifier to get wrong. **Nothing about the failure signal moves.** `terminal_reason = "budget_exhausted"` is still written and is still the sole indicator that the child did not complete naturally: `DelegationResult::success` stays false, `finalize_delegation_result` still tags `[partial — budget_exhausted]`, `relay_status` is still `budget_exhausted_relayed`, and `build_delegate_complete_json` still emits `success: false` with `result_kind: "delegation_failed"`. A consumer parsing those fields sees no change — which is the point: carrying the work is orthogonal to reporting the budget, and conflating them is what the placeholder did. **A child that produced nothing stays distinguishable, twice over:** the two branches are worded differently in prose ("no substantive output was produced before the cap") AND `metadata["cap_carried_content"]` records "true"/"false", following the `relay_status` convention (#E8) so a consumer can branch without matching engine prose. **Deliberately NOT done:** `cap_carried_content` is not hoisted onto `DelegationResult`. The parent already distinguishes the two cases from the summary it receives, and a public struct field with no reader is surface that has to be kept true forever; it can be added when a consumer asks for it.

78. **The thinking-budget hard cut annotates too, through the SAME helper as the cap (v2.13.0, gh#181).** `AgentEngine::hard_cut_budget` pushed an assistant message whose CONTENT was `[thinking budget exhausted — the turn was hard-cut after the completion nudge went unheeded; no tool call was emitted]`, then set `terminal_reason = "budget_exhausted_thinking"`. Same sink as #77 — `DelegationManager::extract_summary` reads the last assistant message — so a child hard-cut by the thinking budget handed its parent that string where its result belonged. Identical defect, other budget path. **The two paths now share one implementation**, `AgentEngine::finish_with_carried_output` + `annotate_hard_stop`: carry the last substantive assistant content, annotate rather than replace, write the caller's `terminal_reason` and the typed `cap_carried_content`, force COMPLETE. They differ only in reason text, in the stop noun ("cap" / "cut") and in the `terminal_reason` value; two hand-rolled copies of the same annotation is how those two paths drift apart, which is the whole reason for one helper. The cap's rendered text is byte-identical to what #77 shipped. **The nothing-was-produced branch matters MORE here, not less.** A hard cut fires precisely when a model narrates instead of calling a tool, so "it produced something substantive first" is less often true on this path than at the iteration cap — which argues for carrying whatever exists and saying plainly when nothing does, not for keeping the placeholder. **`cap_carried_content` is ONE key for both stops, deliberately.** The question a consumer asks is "did the handed-back text contain the agent's own work?", and it should have one place to look; `terminal_reason` already distinguishes which stop fired, and the two values stay distinct — `"budget_exhausted"` for the cap, `"budget_exhausted_thinking"` for the cut — because a consumer may key on them. Nothing else moves: `DelegationResult::success` stays false, the relay still tags `[partial — budget_exhausted]` with `relay_status = "budget_exhausted_relayed"`, and ON_DELEGATE_COMPLETE still reports `success: false` / `result_kind: "delegation_failed"`.

79. **An argument the model is shown must bind something, and when it meets an operator's limit the STRICTER wins (v2.13.0, gh#182).** `data/tools/entropic/delegate.json` advertised `max_turns` to the model with `minimum: 1, maximum: 30` and the description "Maximum number of turns the delegate can use". The value travelled `DelegateDirective` → `PendingDelegation` → `DelegationManager::run_child` → the `delegations` storage row, and stopped: `run_child` dispatches through `run_child_fn_`, which takes no limit, so the child's actual bound came from `LoopConfig::max_iterations` or the child tier's identity frontmatter and never from what the model asked for. `grep max_turns src include` found it only in `storage/backend.cpp` and in `delegation.cpp` signatures — the shape of #76's defect class, an advertised channel wired to nothing, and worse than an absent argument because the model believes it has scoped the work. **The child now carries the request on `LoopContext::delegated_max_turns` and `AgentEngine::resolve_max_iterations` takes `min(operator_limit, delegated_max_turns)`.** A value below the tier's or the engine's bound TIGHTENS it; a value above it changes nothing. A model must not be able to RAISE a limit the operator set — `max_turns` is the model's estimate of how much work the task needs, not a budget grant, and the schema maximum of 30 would otherwise let any lead overrun a tier deliberately pinned at 3. **The min is taken in the RESOLVER, not by folding the request into `effective_max_iterations`.** Two writers touch that field — `apply_identity_overrides` at loop entry and `run_child` before it — so a fold would make the outcome depend on their order, while a min at read time cannot; it also keeps "operator's setting" and "model's request" separately inspectable, which is what the log line and the precedence rule are stated in terms of. `delegated_max_turns < 1` means the argument was omitted, and the resolver is then byte-identical to its pre-gh#182 self, so every un-capped delegation, every pipeline stage and every top-level run behaves exactly as before. **A `max_turns`-bounded child is the ITERATION CAP, not a new terminal.** It reaches the same `resolve_max_iterations` comparison in `should_stop` and `loop`, so it gets #77's hand-back for free: the child's last substantive output is carried and annotated `[iteration cap reached after N iterations …]`, `terminal_reason` stays `"budget_exhausted"`, `DelegationResult::success` stays false and ON_DELEGATE_COMPLETE still reports `success: false` / `result_kind: "delegation_failed"`. Deliberately so — a bounded child did not complete naturally, and inventing a `max_turns_reached` terminal would hand consumers a fourth value to parse for a distinction they can already make from the `max_turns` they themselves sent. **The cost, recorded because it lands on tests:** a model-chosen `max_turns` can now shorten a child the FIXTURE meant to cap elsewhere, so a scenario whose subject is one of the budget terminals must tell its lead not to pass the argument rather than assume it is inert (`make_lead`, `tests/model/test_gh169_gh181_budget_carry.cpp`).

80. **A per-identity override that only a delegated child honours is not a tier setting — it is a delegation setting nobody called one (v2.13.0, gh#183).** `AgentEngine::apply_identity_overrides` reads `max_iterations` and `max_tool_calls_per_turn` from a tier's identity frontmatter and writes them onto the `LoopContext`; it had exactly ONE caller, `run_loop`, which is the door a delegation CHILD arrives through. `AgentEngine::run()` builds its own `LoopContext` and calls `loop(ctx)` directly, so the same tier, with the same frontmatter, was capped when it ran as somebody's delegate and uncapped when it ran as the LEAD. Not a subset of behaviour and not a documented exception — REQ-IDEN-001 said "applied into the LoopContext at loop entry", and `run()` is a loop entry. **Both paths now run ONE preamble**, `AgentEngine::begin_loop_preamble`: apply identity overrides, reinject context anchors, start the metrics clock if it is not already running, move to PLANNING. A second call site was rejected for the same reason gh#181 made the budget cut share gh#169's helper — two copies of a setup sequence drift, and this defect is what the drift looks like after four minor versions. **The interrupt reset and the pause clear stay OUT of the preamble, deliberately.** They are precisely what the two entries must keep doing differently: `run()` is always a fresh top-level turn and resets unconditionally, while `run_loop` resets only when `inherit_interrupt` is false, so a child cannot swallow a parent interrupt raised at dispatch (gh#81). Hoisting them would have handed a top-level run a child's inheritance semantics — the exact failure mode the fix had to avoid, and the reason the preamble is drawn where it is rather than around everything the two functions have in common. `run()`'s own `start_time` assignment also stays where it is, ahead of `init_session_conversation`, so the storage insert keeps counting toward the run's duration as it always has; the preamble's clock start is guarded on `== 0.0` and is a no-op there. **Known and NOT fixed here: a ROUTED lead.** `run(messages, tier_override)` locks the tier up front only when the caller names one (`run_turn_as`, gh#99); with an empty override the tier is chosen inside the first generation by `ResponseGenerator::lock_tier_if_needed`, which is after the preamble has run, so a routed lead's identity overrides still do not apply. Re-applying once the tier is locked would also have to decide what happens when a `dir_tier_change` switches tiers mid-run and the new tier sets no override (the old tier's value is sticky, because `apply_identity_overrides` only writes when the parameter is non-empty). That is a design question about override lifetime, not a missing call, and it is left for its own proposal rather than settled inside a one-line-caller fix.
