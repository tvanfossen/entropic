 ...
 *
 * Replaces a documented fail-open. `GrammarRegistry::get()` returns "" on
 * a miss and the decode proceeds UNCONSTRAINED — which is undetectable
 ...
 * `.gbnf` sat.
 *
 * A tier's stem is STATIC config and is knowable the moment the config is
 * read, so it is an error here. A runtime `params.grammar_key` is not
 * checked: it may name a grammar registered after configure, and its miss
 * stays fail-open, now reported through `generations[].grammar.resolved`.
 *
 * @param config Parsed engine config (tiers carry the stems).
 * @param search_dirs Directories from grammar_search_paths().
 * @return Empty string when every tier stem resolves, else a message
 *         naming the tier, the stem and every directory searched.
 * @req REQ-INFER-007
 * @req REQ-CFG-006
 * @version 2.13.0
 */
ENTROPIC_EXPORT std::string validate_tier_grammars(
    const ParsedConfig& config,
