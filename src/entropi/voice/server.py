...
async def load_models(
    ...
    state.text_tokenizer = sentencepiece.SentencePieceProcessor(
...
async def handle_websocket(websocket, state: ServerState) -> None:
    """
    ...
    # Reset streaming state for new connection
    ...
    # Use asyncio queues for frame buffering between tasks
    input_buffer: asyncio.Queue[bytes | str] = asyncio.Queue(maxsize=10)
    output_buffer: asyncio.Queue[bytes | str] = asyncio.Queue(maxsize=10)
        ...
        try:
            ...
            async for message in websocket:
                if isinstance(message, bytes):
                    ...
                    # Audio frame - queue for processing
                    try:
                    ...
                    # Control message - handle immediately
                    await _handle_control_message(message, state, websocket)
    ...
    async def inference_loop() -> None:
        ...
        try:
            ...
            while not close:
                ...
                try:
                    # Non-blocking get from input buffer
                    message = input_buffer.get_nowait()
                ...
                try:
                    async with state.lock:
                        output_audio, text = await process_audio_frame(state, audio_frame)
                except Exception as e:
                    _log_progress(f"Server inference error: {e}")
                    continue  # Skip this frame but don't close connection
                ...
                if output_audio is not None:
                    ...
                    try:
                        output_buffer.put_nowait(output_audio.tobytes())
                ...
                if text:
                    try:
                        output_buffer.put_nowait(json.dumps({"type": "text", "text": text}))
                    except asyncio.QueueFull:
                        pass

                # Queue stats periodically (every 10 frames)
                if state.stats.frames_processed % 10 == 0:
                    try:
                        output_buffer.put_nowait(json.dumps({
                            "type": "stats",
                            **state.stats.to_dict()
                        }))
                    except asyncio.QueueFull:
            ...
            _log_progress(f"Server inference_loop exiting, processed {frame_count}, output {output_count}")
            close = True
    ...
    async def send_loop() -> None:
        ...
        try:
            ...
            while not close:
                ...
                try:
                    # Non-blocking get from output buffer
                    message = output_buffer.get_nowait()
    ...
    try:
        # Wait for any task to complete (usually recv_loop when client disconnects)
...
async def _handle_control_message(
    ...
    try:
        ...
        if msg_type == "set_prompt":
            ...
            if text_prompt:
                async with state.lock:
                    tokens = state.text_tokenizer.encode(
