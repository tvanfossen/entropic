...
ENTROPIC.md loading, and context compaction for long conversations.
"""
...
class ProjectContext:
    """
    Manages project context from ENTROPIC.md and related files.
    """
    ...
    def __init__(self, project_dir: Path) -> None:
        ...
        self._entropic_md: str | None = None
        self._gitignore_patterns: list[str] = []

    async def load(self) -> None:
        """Load all project context."""
        await self._load_entropic_md()
        await self._load_gitignore()

    async def _load_entropic_md(self) -> None:
        """Load ENTROPIC.md if present (from .entropic/ folder)."""
        # Primary location: .entropic/ENTROPIC.md
        entropic_path = self.project_dir / ".entropic" / "ENTROPIC.md"
        if entropic_path.exists():
            self._entropic_md = entropic_path.read_text()
            logger.info(f"Loaded ENTROPIC.md from {entropic_path}")
...
The following is project-specific context from ENTROPIC.md:
...
"""
    ...
    def has_context(self) -> bool:
        ...
        return self._entropic_md is not None
