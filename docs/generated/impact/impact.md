...
|-----|------|-------------------|
| REQ-ABI-001 | Pure C at every .so boundary — opaque handles and explicit ownership | entropic_destroy, entropic_state_save, entropic_state_load |
...
| REQ-API-005 | Uniform precondition guard on every exported entry point | entropic_run_batch, entropic_state_save, entropic_state_load, entropic_adapter_load, entropic_get_logprobs, entropic_compute_perplexity, entropic_model_has_vision, entropic_release_model, entropic_workspace_create, entropic_session_bind_workspace, entropic_register_mcp_server |
...
| REQ-CFG-006 | Fail-loud validation — reject bad or inert configuration at load time | parse_config_string, warn_unresolved_tier_grammars |
...
| REQ-LOOP-001 | Agent state machine with observable, dual-channel transitions | run_loop, run, run_turn, entropic_run_session |
...
| REQ-LOOP-009 | Per-session-key run concurrency, on by default, with one lock on the generation path | RunSessionScope, plan_batch_kv, plan_disturbs_sessions, current_run_session, RunSessionScope, ~RunSessionScope |
...
| REQ-MCP-007 | Tool calls route by server prefix uniformly across all three server kinds | query_tools, init_builtins, tools_without_readonly_hint, external_tools_without_readonly_hint |
...
| REQ-MCP-021 | Filesystem tools are root-confined, read-before-write gated, and size-bounded | confined_mcp_config, collect_glob_matches, execute |
...
**Total: 66 requirement(s) affected, 290 function(s) changed**
